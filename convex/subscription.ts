...
export const hasEntitlement = query({
    ...
    handler: async (ctx, { userId }) => {
        ...
        for await (const sub of ctx.db
            ...
            const period0k = sub.currentPeriodEnd == null || sub.currentPeriodEnd > now
...
export const upsertFromPolar = mutation({
    ...
    handler: async (ctx, args) => {
        console.log('Starting upsertFromPolar for - ', {
            userId: args.userId,
        ...
        const base = {
            ...
            creditsGrantPerPeriod:
                args.creditsGrantPerPeriod ?? existingByPolar?.creditsGrantPerPeriod ?? existingByUser?.creditsGrantPerPeriod ?? DEFAULT_GRANT,
            creditsRolloverLimit:
                args.creditsRolloverLimit ?? existingByPolar?.creditsRolloverLimit ?? existingByUser?.creditsRolloverLimit ?? DEFAULT_ROLLOVER_LIMIT,
        }
        ...
        if (existingByPolar) {
            if (existingByPolar.userId === args.userId) {
                ...
                if (userExistingSubscription) {
                    ...
                    console.log('Convex: Created subscriptions: ', newId)
                    return newId
                }
            }
        }
        ...
        console.log('Convex: Creating new subscriptions for user: ', args.userId)
        const newId = await ctx.db.insert('subscriptions', {
            ...base,
            creditsBalance: 0,
            lastGrantCursor: undefined,
...
// idempotencyKey -> A temporary key that is used to verify that a transaction only happens once. 
