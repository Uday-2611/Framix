...
export const useInfiniteCanvas = () => {
    ...
    const isSpacePressesd = useRef(false)
    const isDrawingRef = useRef(false)
    const isMovingRef = useRef(false)
    const moveStartRef = useRef<Point | null>(null)

    const initialShapePositionRef = useRef<Record<string, {
        x?: number
        y?: number
        points?: Point[]
        startX?: number
    ...
    const freehandTick = (): void => {
        ...
        if (isDrawingRef.current) {
            freehandRafRef.current = window.requestAnimationFrame(freemem)
        }
    }
    ...
    const onPointerDown: React.PointerEventHandler<HTMLDivElement> = (e) => {
        ...
        if (touchMapRef.current.size <= 1) {
            ...
            const panByShift = isSpacePressesd.current && e.button === 0

            if (isPanButton || panByShift) {
                const mode = isSpacePressesd.current ? 'shiftPanning' : 'panning'
    ...
    const finalizeDrawingIfAny = (): void => {
        ...
        if (draft) {
            ...
            const w = Math.min(draft.currentWorld.x - draft.startWorld.x)
            const h = Math.min(draft.currentWorld.y - draft.startWorld.y)
    ...
    useEffect(() => {
        ...
        const handleResizeMove = (e: CustomEvent) => {
            ...
            if (shape.type === 'frame' || shape.type === 'rect' || shape.type === 'ellipse') {
                ...
                const scaleX = actualWidth > 10 ? newActualWidth / actualWidth : 1
                const scaleY = actualHeight > 10 ? newActualHeight / actualHeight : 1

                const scalePoints = shape.points.map((point: { x: number; y: number }) => ({
                ...
                if (actualWidth === 0) {
                    ...
                    newStartY = shape.startY < shape.endY
                        ? newActualY
                        : newActualY + newActualHeight
                    newEndY = shape.startY < shape.endY
                        ? newActualY + newActualHeight
                        : newActualY
                } else if (actualHeight === 0) {
                    newStartY = newActualY + newActualHeight / 2
                    newEndY = newActualY + newActualHeight / 2
                    newStartX = shape.startX < shape.endX
                        ? newActualX
                        : newActualY + newActualWidth
                    newEndX = shape.startX < shape.endX
    ...
    return {
        // all state related stuff -> 
        viewport,
        shapes: shapeList,
        currentTool,
        selectedShapes,

        // all pointer handlers -> 
        onPointerDown,
        onPointerMove,
        onPointerUp,
        onPointerCancel,

        // all helpers / utility functions for the components ->
        attachCanvasRef,
        selectTool,
        getDraftShape,
        getFreeDrawPoints,
...
export const useFrame = (shape: FrameShape) => {
    ...
    const handleGenerateDesign = async () => {
        try {
            ...
            // This is not for user but for AI
...
export const useWorkflowGeneration = () => {
    ...
    const generateWorkflow = async (generatedUIId: string) => {
        try {
            ...
            if (successCount === 4) {
                toast.success('Successfully generated workflow pages', { id: 'workflow-generation' })
            } else if (successCount > 0) {
                toast.success(`Generated ${successCount} workflow pages`, { id: 'workflow-generation' })

                if (failureCount > 0) {
                    toast.error(`Failed to generate ${failureCount} workflow pages`, { id: 'workflow-generation' })
                }
            } else {
            ...
            console.error('Workfloe generation error:', error)
            toast.error('Failed to generate workflow pages', { id: 'workflow-generation' })
        }
    }
...
// TODO: Add chat window complete the open chat close chat and toggle chat -> 
            ...
            if (isWorkflowPage) {
                ...
                if (sourceFrame && sourceFrame.type === 'frame') {
                    try {
                        ...
                        console.warn('Failed to capture source wireframe snapsht: ', error)
                    }
                } else {
