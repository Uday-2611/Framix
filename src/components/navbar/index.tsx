                    ...
                    {tabs.map((t) => (
                        <Link
                            ...
                            className={[
                                ...
                                `${pathname}?project=${projectId}` === t.href ? 'bg-white/[0.12] text-white border border-white/[0.16] backdrop-blur-sm' : ' text-zinc-400 border border-transparent hover:text-zinc-200 hover:bg-white/[0.06]'
                            ].join(' ')}
                        >
                            <span
                                className={
...
// Redux -> Very similar to context API except it has a more standardized process and is used in most businesses as the global state management.
// Redux Toolkit Query - RTK Query - A tool inside redux toolkit and its job is to talk to servers and database and act as a messenger between app and database, without it we would need to make a fetch, handle load, handle errors, handle caching and also handle updating the UI when data changes. 