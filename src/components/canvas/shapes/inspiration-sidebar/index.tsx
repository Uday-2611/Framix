        ...
        <div className={cn('fixed left-5 top-1/2 transform -translate-y-1/2 w-80 backdrop-blur-xl bg-white/[0.08] border-white/[0.12] gap-2 p-3 saturate-150 border rounded-lg z-50 transition-transform duration-300')}>
            <div className='p-4 flex flex-col gap-4 overflow-y-auto max-h-[calc(100vh-8rem)]'>
                <div className='flex items-center justify-between'>
                    ...
                    <Label className='text-white/80 font-medium'></Label>
                </div>
                <Button
            ...
            {images.length > 0 && (
                <div className='space-y-3'>
                    <div className='flex items-center justify-between'>
                        <Label className='text-white/80 text-sm'>Upload Images ({images.length})</Label>
                        <Button
                            variant='ghost'
                            size='sm'
                            ...
                            <div key={image.id} className='relative group aspect-square rounded-lg overflow-hidden border border-white/10 bg-white/5'>
                                ...
                                {image.uploading && (
                                    <div className='absolute inset-0 bg-black/50 flex items-center justify-center'>
                                        <Loader2 className='w-5 h-6 text-white animate-spin' />
                                    </div>
                                )}

                                {image.error && (
                                    <div className='absolute inset-0 bg-red-500/20 flex items-center justify-center'>
                                        <p className='text-xs text-rose-300 text-center px-2'>
                                            {image.error}
                                        </p>
                                    </div>
                                )}
                                ...
                                <Button
                                    ...
                                    className='absolute top-1 right-1 h-6 w-6 p-0 bg-black/50 hover:bg-background/70 opacity-0 group-hover:opacity-100 transition-opacity'
                                >
