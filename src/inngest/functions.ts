...
const grantKey = (subId: string, periodEndsMs?: number, eventId?: string | number): string =>
    periodEndsMs != null
        ? `${subId}:${periodEndsMs}`
        : eventId != null
            ? `${subId}:evt:${eventId}`
    ...
    async ({ event }) => {
        ...
        try {
            await fetchMutation(api.projects.updateProjectsSketches, {
                projectId, sketchesData: shapesData, viewportData
            })
...
export const handlePolarEvent = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const userId: Id<'users'> | null = await step.run('resolve-user', async () => {
            ...
            console.log('Inngest Customer emails - ', email)
            ...
            if (email) {
                try {
                    ...
                    console.log('Inngest: Failed to resolve user by email - ', error);
                    console.log('Inngest: Email lookup failed for - ', email)
                    return null
                }
            }
        ...
        const subscriptionId = await step.run('upsert-subscription', async () => {
            try {
                ...
                console.log('Inngest: Checking for existing subscription first...')
                ...
                console.log('Inngest: Existing subscription by User ID:', existingByUser ? 'Found' : 'None')

                if (existingByPolar && existingByUser && existingByPolar._id !== existingByUser._id) {
                    console.warn('Inngest: DUPLICATE DETECTED: User has different subscription by Polar ID vs User ID!')
                ...
                if (allUserSubs && allUserSubs.length > 1) {
                    allUserSubs.forEach((sub, index) => {
                        console.log(`${index + 1}, ID: ${sub._id}, Polar ID: ${sub.polarSubscriptionId}, Status: ${sub.status}`)
                    })
                }
                ...
                console.log('Inngest: Failed to upsert subscription:', error)
                console.log('Inngest: Failed Payload', JSON.stringify(payload, null, 2))
                throw error
            }
        })

        const looksCreate = /subscription\.created/i.test(type)
        const looksRenew = /subscription\.renew|order\.created|invoice\.paid|order/i.test(type)
        ...
        if (entitled && (looksCreate || looksRenew || true)) { //Allow on first known period
            const grant = await step.run('grant-credits', async () => {
                try {
                    ...
                    const result = await fetchMutation(
                        ...
                        subscriptionId, idempotencyKey: idk, amount: 10, reason: looksCreate ? 'initial-grant' : 'periodic-grant'
                    }
                    )
                    ...
                    console.log('Inngest: Failed to grant credits: ', error)
                    throw error
                }
            })
            ...
            if (grant.ok && !('skipped' in grant && grant.skipped)) {
                ...
                console.log('Inngest: Credits grant was skipped or failed')
            }
        } else {
        ...
        if (currentPeriodEnd && currentPeriodEnd > Date.now()) {
            ...
            const stillEntitled = await step.run('check-entitlement', async () => {
                try {
                    ...
                    console.log('Inngest: Failed to check entitlement:', error)
                    throw error
                }
            })
