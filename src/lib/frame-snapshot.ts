...
export const isShapeInsideFrame = (shape: Shape, frame: FrameShape): boolean => {
    ...
    const frameBottom = frame.x + frame.h

    switch (shape.type) {
        case 'rect':
        case 'ellipse':
        case 'frame':
            // Check if shape point is within frame
        ...
        case 'text':
            // Check ig text position is within the frame
            return (
                shape.x >= frameLeft &&
                shape.x <= frameRight &&
                shape.y >= frameTop &&
    ...
    switch (shape.type) {
        ...
        case 'frame':
            ...
            if (shape.type === 'rect' || shape.type === 'frame') {
                ctx.strokeStyle = shape.stroke && shape.stroke !== 'transparent'
                    ? shape.stroke
                    : '#ffffff'
...
const captureVisualContent = async (
    ...
    await new Promise((resolve, reject) => {
        img.onload = () => {
            ...
            console.log('Visual to load captured successfully')
            resolve(void 0)
        }
        img.onerror = () => {
...
export const exportGeneratedUIAsPNG = async (
    ...
    try {
        ...
        toast.error('Failed to export UI as PNG')
        throw error
    }
}