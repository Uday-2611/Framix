...
export async function POST(request: NextRequest) {
    try {
        ...
        if (wireframeSnapshot) {
            ...
            console.log('No wireframe context available - using text-only generation')
        }
        ...
        if (colors.length > 0) {
            ...
            ).map((color) => color.swatches.map((swatch) =>
        ...
        if (typography.length > 0) {
            ...
            ).map((typo) => typo.styles.map((style) =>
                `${style.name}: ${style.fontFamily}, ${style.fontSize}, ${style.lineHeight}, ${style.description}, ${style.fontWeight}`
            )
                .join(', ')
            )
                .join(', ')}`
