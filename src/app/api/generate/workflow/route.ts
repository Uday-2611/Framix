        ...
        if (typography.length > 0) {
            userPrompt += `\n\nTypography:\n${(
                ...
                typo.styles.map((style) =>
                    `${style.name}: ${style.description}, ${style.fontFamily}, ${style.fontSize}, ${style.lineHeight}, ${style.fontWeight}`
                ).join(', ')
            ).join(', ')}`
        }

        if (imagesUrls.length > 0) {
            userPrompt += `\n\nInspiration Images Available: ${imagesUrls.length} references images for visual style and inspiration`
        }
...
// TODO: Use SHadCN components to make the designs. 